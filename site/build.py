...
"""Build the Meridian Advisory bespoke site.
...
Usage:
    ...
    ./site/build.py serve        # build then serve on :8006 for local preview
...
def build():
    print(f"Building Meridian Advisory → {DIST_DIR}")
    ...
    pages = [
        ...
        ("practices.html.j2", "practices.html"),
    ...
    if not cname_file.exists():
        cname_file.write_text("meridianadvisory.eduserver.au\n", encoding="utf-8")
        print("  ✓ CNAME")
...
def serve():
    """Build then serve the dist directory on :8006 for local preview."""
    import http.server
    import socketserver
    import os
    ...
    with socketserver.TCPServer(("", PORT), http.server.SimpleHTTPRequestHandler) as httpd:
        print(f"\nServing Meridian at http://localhost:{PORT}")
